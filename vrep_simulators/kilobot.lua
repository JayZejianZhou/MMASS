...
4 if(distance < 0.033) then
    ...
    if (sim.getScriptExecutionCount()==0) then 
        ...
        --////////////////////////////////////////////////////////////////////////////////////
            ...
            -- send message
            --sim.sendData(sim.handle_all,0,"Message","Kilobot"..fullname.." msg:"..i,MsgSensorsHandle,0.1,3.1415,3.1415*2)    
            -- message_out(2,3,4)
            ...
            if (distance ~= nil)  then 
                ...
                if ((kname== "BatHold") or (kname== "Base")) then
                    ...
                    -- move CC Waround  the detected robot
        ...
        -----------blink the LED
    function _blink(color_r, color_g, color_b, delay_time)
        if (substate==0) then    
        ...
        elseif (substate==3) then
            if (_delay_ms(delay_time)==1) then
                substate=0
            end
        end
    end
    ...
    --simRemoteApi.start(20001) --zz: does not work, report not valid number
    
    
    --sim.addStatusbarMessage("special_mode"..tostring(special_mode)) 
    if(special_mode ~= 1) then
        --not special mode, LED green
        set_color(0,3,0)
    ...
    if(special_mode==1) then
        set_color(3,0,0)
        --sim.addStatusbarMessage(sim.getScriptName(sim.handle_self).." manual overide") 
        run_program=0
